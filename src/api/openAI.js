...
const resultl = uploadBytes(imageRef, file).then((snapshot) => {
...
const resizeAndHostImage = async (hostedImage) => {
...
export async function generatePostBody(cruiseInfo,cruisLine,shipName,destination) {
  //console.log(cruiseInfo)
  //openai.listModels();
  //return
  const cleanText = cleanYouTubeTranscript(cruiseInfo);
  const prompt = postBodyPrompt(cleanText,cruisLine,shipName,destination);
  console.log("Here is the prompt", prompt);
  return davinci(prompt);
}

export function generateExtraInfo(info,cruisLine,cruiseShip,destination){
 const prompt = extraInfoPrompt(info);
 return davinci(prompt,0.1,cruisLine,cruiseShip,destination) 
}
...
export const featuresPrompt = (data) => {
  return `
  ${rolePrompt}
  
  Context: 
  Special offers are things like "Dining Specials","Onboard credits","special drink packages, and anything that is offered by the Cruise Line as an "Added Value"" not always available.

  Task: 
  Make a list of any special offers and deals included with this cruise from the following data.
  If you don't discover anything you think can be considered a "Special offer" just return nothing. Don't make anything up.
  
  Output: 
  For each of the special offers in the list, create a JSON object with the following structure:
  {header: "string", image: "string",details: "string"}
  For header, write a one line description of the special.
  For image, write a string containing a list of 3 comma separated keywords that can be used for creating or searching images. 
  For details, write an interesting summary of the special offer in 30 words or less.
...
export const imagePrompt = (humanPrompt) => {
return `
${humanPrompt} --exclude-text --exclude-words --exclude-people
`
}
// export const imagePrompt = (humanPrompt) => {
// return `
// ${humanPrompt} --digital-art --abstract --exclude-text --exclude-words --exclude-people
// `
// }

export const extraInfoPrompt = (info) => {

  return `
    Task: Sort through the following data, find all cruise amenity details ,and return a list of titles and one line descriptions about them.
    Output: Return an array of JSON object in this format - {title: "",
    info: []}

  `
  // {title: "In room dining special",
  // info: ["Free for all suites 5 nights or more","Available for 2 nights"]}
}
